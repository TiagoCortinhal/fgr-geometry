"""
Retrain canonical config-B GRU-VAE with CORRUPT BIOMETRY MASKED.

Config B: per-visit seq = biometry(5 z + 5 mask) + lag(scalar+mask) + GA = Din 13;
GRU H=32; Z=8; beta=0.1; unsupervised (reconstructs seq-mean). ORIGINAL lag (_lag_seq.npz).
birth-pct / SGA / LGA are eval-only.

Data hygiene: any biometry z-score < -10 is physiologically impossible (traced to bad raw
eco-visit measurements, e.g. fetus 528 head 10.1 cm at 36 wk). We MASK those cells: set the
z value to 0 and its biometry-mask bit to 0, so the model treats that single measurement as
missing. The fetus's good measurements are kept. (25 corrupt cells across 25 fetuses.)

Saves clean full-data latent _lagB_traj_clean.npy (908,12,8) and 5-fold OOF metrics.
"""
...
def build_inputs():
    ...
    # --- MASK corrupt biometry cells (z < -10) ---
    biomz = X[:, :, :F]; corrupt = biomz < -10
    n_corrupt = int(corrupt.sum())
    for j in range(F):
        cj = corrupt[:, :, j]
        X[:, :, j][cj] = 0.0            # zero the impossible z value
        X[:, :, F + j][cj] = 0.0        # clear its mask bit -> treated as missing
    ...
    return Xc, L, fids, n_corrupt


def train_eval(epochs=250, H=32, Z=8, beta=0.1):
    Xc, L, fids, n_corrupt = build_inputs()
    ...
    np.save(f"{IMG}/_lagB_traj_clean.npy", traj)
    return dict(tag="config-B original lag, corrupt biometry masked", n_corrupt_cells=n_corrupt,
                Din=int(Din), eff_dim=round(eff_dim(Zoof), 3), birthpct_r=round(r_bp, 3),
                SGA_auc=round(auc(sga), 3), LGA_auc=round(auc(lga), 3), traj="_lagB_traj_clean.npy")


if __name__ == "__main__":
    torch.manual_seed(0); np.random.seed(0)
    res = train_eval()
    json.dump(res, open(f"{IMG}/lag_clean_gruvae_results.json", "w"), indent=2)
