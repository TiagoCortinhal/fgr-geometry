...
class ShardFeeder:
    ...
    def _stats(s):
        ...
        for i,f in enumerate(s.fs):
            pt=np.load(f,allow_pickle=True)["tokens"][:,:,1:,:].astype(np.float32)  # (n,L,Np,D)
            x=pt.transpose(0,2,1,3).reshape(-1,s.L,s.D)
            if a is None: a=np.zeros((s.L,s.D)); b=np.zeros((s.L,s.D))
            a+=x.sum(0); b+=(x**2).sum(0); n+=len(x); del pt,x
            if i%15==0: print(f"    stats shard {i+1}/{len(s.fs)} {time.time()-t0:.0f}s",flush=True)
        s.mu=(a/n).astype(np.float32); s.sd=(np.sqrt(np.maximum(b/n-(a/n)**2,1e-12))+1e-6).astype(np.float32)
    def _map(s,tok):
        if s.all:
            pt=tok[:,:,1:,:].astype(np.float32); n,L,Np,D=pt.shape
            pt=(pt.transpose(0,2,1,3)-s.mu[None,None])/s.sd[None,None]   # z-score (n,Np,L,D)
            fm=pt.reshape(n,Np,L*D)
        else:
            fm=tok[:,s.layer,1:,:].astype(np.float32)                    # (n,Np,D)
        n,Np,C=fm.shape; g=int(round(Np**0.5))
        return fm.reshape(n,g,g,C).transpose(0,3,1,2)                    # (n,C,g,g)
